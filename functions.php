...
/**
 * dynamic_start Theme functions and definitions
 *
 * @file      functions.php
...
/**
 * Tell WordPress to run dynamic_start_setup() when the 'after_setup_theme' hook is run.
 */

add_action('after_setup_theme', 'dynamic_start_setup');

if (!function_exists('dynamic_start_setup')) :

	function dynamic_start_setup()
	{
		...
		register_nav_menus(array(
			'primary' => __('Menu topo', 'dynamic_start'),
			'menumobili' => __('Menu Mobili', 'dynamic_start'),
		));
		
		function dynamic_start_menu_fallback()
		{ ?>
			<ul class="menu">
				<?php
				wp_list_pages(array(
		...
		function dynamic_start_menu_fallback_vg_nav()
		{ ?>
...
function thema_styles()
{
	...
	wp_enqueue_style('core', get_template_directory_uri() . '/assets/scss/index.css', array(), '1.1', 'all'); 
}
...
function theme_scripts()
{
	...
	//add JS defalt theme
	wp_enqueue_script('dynamic_start_custom', get_template_directory_uri() . '/assets/js/custom.js', array('jquery'), '1.8', true);
}
...
if (function_exists('register_sidebar')) {
	...
	register_sidebar(array(
		'name' => __('Barra leteral', 'dynamic_start'),
		'id' => 'sidebar-1',
	...
	register_sidebar(array(
		'name' => __('Footer Area', 'dynamic_start'),
		'id' => 'sidebar-2',
		'description' => __('Uma área de widget opcional para o rodapé do seu site', 'dynamic_start'),
...
function show_category_post()
{
	...
	if ($categories) {
		...
		echo '<h3>' . __('Categoria:', 'dynamic_start') . '</h3>';
 ...
 *
 */
if (!function_exists('dynamic_start_comments')) {

	function dynamic_start_comments($comment, $args, $depth)
	{
		...
		switch ($comment->comment_type):
			...
			?>
				<li class="pingback">
					<span class="title"><?php _e('Pingback:', 'dynamic_start') ?></span> <?php comment_author_link(); ?>
				<?php
					...
					<div id="comment-<?php comment_ID(); ?>" class="comment">
						<div class="comment-meta">
							...
							<div class="comment-author vcard">
								...
								printf(
									...
									sprintf(
										...
										sprintf(__('%1$s at %2$s', 'dynamic_start'), get_comment_date(), get_comment_time())
									)
								);
								?>
							...
							<?php if ($comment->comment_approved == '0') : ?>
								<em class="comment-awaiting-moderation"><?php _e('Your comment is awaiting moderation.', 'dynamic_start'); ?></em>
								<br />
							<?php endif; ?>
						...
						<div class="reply">
							<?php comment_reply_link(array_merge($args, array('reply_text' => __('Reply', 'dynamic_start'), 'depth' => $depth, 'max_depth' => $args['max_depth']))); ?>
						</div><!-- ./reply -->
					</div><!-- /comment -->
				<?php
				break;
 ...
 */
function dynamic_start_pagination()
{
	global $wp_query;
